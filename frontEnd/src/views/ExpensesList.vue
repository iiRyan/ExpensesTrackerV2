                ...
                <div class="w-full">
                    <div class="border-b border-gray-200 shadow">
                        <table class="divide-y divide-green-400 ">
                            ...
                            <tbody class="bg-white divide-y divide-gray-300" v-for="(expense, index) in expenses"
                                :key="index">
                                <tr class="whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-gray-500"> {{ expense.attributes.amount }}</div>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-500">
                                        {{ expense.attributes.bank_account }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <button>
                      ...
                      2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    ...
                                    <td class="px-6 py-4">
                                        <button @click.prevent="deleteExpense(expense.id)">
...
const total = computed(() => {
    return expenses.value.reduce((acc, expense) => {
        return acc + Number(expense.attributes.amount);
    }, 0);
});

const left = computed(() => {
    return expenses.value.reduce((acc, expense) => {
        return acc + Number(expense.relationships.income - expense.attributes.amount);
    }, 0);
});
...
const loadData = async () => {
    ...
    const response = await axios.get(`http://127.0.0.1:8000/api/expenses/month/${monthId}`, {
        headers: { Authorization: `Bearer ${store.token}` },
    });
